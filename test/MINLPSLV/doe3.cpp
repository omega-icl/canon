...
public:
  ...
  void eval
    ...
    const
    {
      ...
      //std::cout << Amat;
      //if( CPPL::dgeqrf( Amat.to_dgematrix(), vRes[0] ) || vRes[0] <= 1 )//machprec() )
      if( CPPL::dgeqrf( Amat.to_dgematrix(), vRes[0] ) )//machprec() )
...
public:
  ...
  void eval
    ...
    const
    {
      ...
      if( CPPL::dsytrf( Amat, Lmat, IPIV ) )
        throw FFBase::Exceptions( FFBase::Exceptions::EXTERN );
      static CPPL::dgematrix Xmat;
      for( unsigned i=0; i<nVar; ++i ){
        if( CPPL::dsytrs( Lmat, IPIV, _A->at(i).to_dgematrix(), Xmat ) )
          throw FFBase::Exceptions( FFBase::Exceptions::EXTERN );
        //std::cout << Xmat;
