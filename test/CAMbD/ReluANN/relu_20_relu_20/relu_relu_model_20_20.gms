...
$GDXIN Relu_20_20_20_sle.gdx
$load snf sn_l1 sn_l2 sn_l3 sn_l4 sw_l1 sw_l2 sw_l3 sw_l4 sb_l1 sb_l2 sb_l3 sb_l4 sscaler_mean sscaler_var
$GDXIN


VARIABLES
sinps(snf)    'calculate the scaled input features'
snn_sum_l1(sn_l1)
snn_sum_l2(sn_l2)
snn_sum_l3(sn_l3)
snn_sum_l4(sn_l4)
...
sinps.l('qa') = 1/sscaler_var('qa')*(qc.l('s1') - sscaler_mean('qa'));
sinps.l('qb') = 1/sscaler_var('qb')*(qc.l('s2') - sscaler_mean('qb'));
sinps.l('ra') = 1/sscaler_var('ra')*(rc.l('s1') - sscaler_mean('ra'));
sinps.l('rb') = 1/sscaler_var('rb')*(rc.l('s2') - sscaler_mean('rb'));
sinps.l('za') = 1/sscaler_var('za')*(x.l('s1') -  sscaler_mean('za'));
sinps.l('zb') = 1/sscaler_var('zb')*(x.l('s2') -  sscaler_mean('zb'));


$macro relu(x) (x/2 + abs(x)/2)
EQUATIONS
seq_sur1,seq_sur2,seq_sur3,seq_sur4,seq_sur5
seq_sur6,seq_sur7,seq_sur8,seq_sur9,seq_sur10
seq_sur11,seq_sur12,seq_sur13,seq_sur14,seq_sur15
;

* ann max operator in seq_sur8 and sigmoid function in seq_sur11

****** surrogate model for stability check ******

* prepare the input feature 
seq_sur1..
sinps('qa') =e= 1/sscaler_var('qa')*(qc('s1') - sscaler_mean('qa'));

seq_sur2..
sinps('qb') =e= 1/sscaler_var('qb')*(qc('s2') - sscaler_mean('qb'));

seq_sur3..
sinps('ra') =e= 1/sscaler_var('ra')*(rc('s1') - sscaler_mean('ra'));

seq_sur4..
sinps('rb') =e= 1/sscaler_var('rb')*(rc('s2') - sscaler_mean('rb'));

seq_sur5..
sinps('za') =e= 1/sscaler_var('za')*(x('s1') - sscaler_mean('za'));

seq_sur6..
sinps('zb') =e= 1/sscaler_var('zb')*(x('s2') - sscaler_mean('zb'));

* relu ann calculation 
seq_sur7(sn_l1)..
snn_sum_l1(sn_l1) =e= sum(snf,sw_l1(snf,sn_l1)*sinps(snf)) + sb_l1(sn_l1);

seq_sur8(sn_l1)..
snn_act_l1(sn_l1) =e= relu(snn_sum_l1(sn_l1));

seq_sur9(sn_l2)..
snn_sum_l2(sn_l2) =e= sum(sn_l1,sw_l2(sn_l1,sn_l2)*snn_act_l1(sn_l1)) + sb_l2(sn_l2);

seq_sur10(sn_l2)..
snn_act_l2(sn_l2) =e= relu(snn_sum_l2(sn_l2));

seq_sur11(sn_l3)..
snn_sum_l3(sn_l3) =e= sum(sn_l2,sw_l3(sn_l2,sn_l3)*snn_act_l2(sn_l2)) + sb_l3(sn_l3);

seq_sur12(sn_l3)..
snn_act_l3(sn_l3) =e= relu(snn_sum_l3(sn_l3));

seq_sur13(sn_l4)..
snn_sum_l4(sn_l4) =e= sum(sn_l3,sw_l4(sn_l3,sn_l4)*snn_act_l3(sn_l3)) + sb_l4(sn_l4);

seq_sur14(sn_l4)..
snn_act_l4(sn_l4) =e= 1/(1+exp(-snn_sum_l4(sn_l4)));

seq_sur15(sn_l4)..
snn_act_l4(sn_l4) =g= 0.5;
